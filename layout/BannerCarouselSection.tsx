...
export interface BannerCarouselSectionData {
  heading: string;
  text: string;
  bannerCarouselItems: BannerCarouselItem[];
}

interface BannerCarouselSection extends BannerCarouselSectionData {
  ctaContent?: string;
  ctaHref?: string;
...
const BannerCarouselSection: React.FC<BannerCarouselSection> = ({ heading, text, ctaContent, ctaHref, classNameTextStructure, headingColor, textColor, textMaxWidth, bannerCarouselItems = [] }) => {
  return (
    <section className='@container h-fit w-full py-15 lg:py-20 grid grid-cols-4 sm:grid-cols-12 gap-x-4 md:gap-x-5 lg:gap-x-7.5 bg-backgroundSecondary'>
      <SectionHeader className='col-span-4 sm:col-span-12' heading={heading} text={text} ctaContent={ctaContent} ctaHref={ctaHref} classNameTextStructure={classNameTextStructure} headingColor={headingColor} textColor={textColor} textMaxWidth={textMaxWidth} />
