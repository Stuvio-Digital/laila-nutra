...
const FeatureGridSection: React.FC<FeatureGridSectionProps> = ({ heading, text, ctaContent, ctaHref, classNameTextStructure, headingColor, textMaxWidth, textColor, bgColor, features }) => {
  ...
  return (
    <section className={`@container h-fit w-full py-15 lg:py-20 grid ${isMultipleOfFive ? "grid-cols-5" : "grid-cols-4"} gap-x-4 md:gap-x-5 lg:gap-x-7.5 bg-${bgColor} `}>
      ...
      <div className={`col-span-full grid ${isMultipleOfFive ? "grid-cols-4 @4xl:grid-cols-5" : "grid-cols-4"} gap-x-4 md:gap-x-5 lg:gap-x-7.5 px-4 sm:px-6 lg:px-10 gap-y-6 sm:gap-y-7 md:gap-y-10`}>
        {features.map((feature, index) => (
          <FadeUp key={index} className={`${isMultipleOfFive ? "col-span-2 sm:col-span-1 @4xl:col-span-1" : "col-span-4 sm:col-span-2 @4xl:col-span-1"} flex flex-col pt-6 @4xl:pt-0 @4xl:border-t-0 border-t border-borderColor sm:border-t-0`}>
